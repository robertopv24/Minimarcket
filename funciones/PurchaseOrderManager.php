...
class PurchaseOrderManager {
    ...
    public function getAllPurchaseOrders() {
        try {
            $stmt = $this->db->query("SELECT * FROM purchase_orders");
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error al obtener todas las órdenes de compra: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function addItemToPurchaseOrder($purchaseOrderId, $productId, $quantity, $unitPrice) {
        try {
            $stmt = $this->db->prepare("INSERT INTO purchase_order_items (purchase_order_id, product_id, quantity, unit_price) VALUES (?, ?, ?, ?)");
            return $stmt->execute([$purchaseOrderId, $productId, $quantity, $unitPrice]);
        } catch (PDOException $e) {
