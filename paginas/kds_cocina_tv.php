...
$ordenesFiltradas = getKDSData('kitchen');
...
function renderTicket($data)
{
    ...
    <div class="ticket <?= $borderClass ?> <?= ($data['is_mixed'] ?? false) ? 'ticket-mixed' : '' ?> <?= $isPrep ? 'status-preparing-active' : '' ?>"
        ...
        <div class="ticket-head <?= $bgStatus ?>"
            onclick="updateStatus(<?= $orden['id'] ?>, '<?= $targetStatus ?>', 'kitchen')">
            <div class="d-flex align-items-center">
                ...
                <?php if ($orden['is_other_ready']): ?>
                    <i class="fa fa-pizza-slice ms-2 text-danger" title="Pizza ya está lista"></i>
                <?php endif; ?>
            </div>
