...
foreach ($ordenes as $orden) {
    ...
    foreach ($items as $item) {
        ...
        for($i = 0; $i < $loop; $i++) {
            ...
            $ingredientes = [];
            foreach($currentMods as $m) {
                if($m['modifier_type'] == 'remove') $ingredientes[] = "NO " . $m['ingredient_name'];
                if($m['modifier_type'] == 'add') $ingredientes[] = "EXTRA " . $m['ingredient_name'];
            }

            $note = "";
            if($i==0) {
                foreach($mods as $gm) {
                    if($gm['sub_item_index'] == -1 && $gm['modifier_type'] == 'info') $note = $gm['note'];
                }
            }
    ...
    if (!empty($itemsParaPizza)) $listaPizza[] = ['info' => $orden, 'items' => $itemsParaPizza];
    if (!empty($itemsParaCocina)) $listaCocina[] = ['info' => $orden, 'items' => $itemsParaCocina];
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        body { background-color: #1a1a1a; color: white; overflow: hidden; height: 100vh; font-family: 'Segoe UI', sans-serif; }
        .station-col { height: 100vh; overflow-y: auto; padding: 10px; border-right: 2px solid #444; }

        .station-header {
            font-weight: 900; text-align: center; text-transform: uppercase;
            padding: 15px; margin-bottom: 20px; border-radius: 8px; font-size: 1.5rem; letter-spacing: 2px;
            position: sticky; top: 0; z-index: 10; box-shadow: 0 4px 10px rgba(0,0,0,0.5);
        }
        .header-pizza { background: linear-gradient(45deg, #d32f2f, #b71c1c); }
        .header-cocina { background: linear-gradient(45deg, #f57f17, #e65100); }

        .ticket {
            background: #fff; color: #000; border-radius: 8px; margin-bottom: 15px;
        ...
        .status-preparing { background-color: #ffca28; color: #000; }

        .ticket-body { padding: 10px 15px; }
        .main-item { font-size: 1.2rem; font-weight: 800; border-bottom: 2px solid #eee; margin-top: 10px; padding-bottom: 5px; }
        .sub-item {
            margin-top: 5px; padding-left: 10px; border-left: 3px solid #ccc; font-size: 1rem;
            display: flex; flex-direction: column;
        }

        .tag { padding: 2px 6px; border-radius: 4px; font-weight: bold; font-size: 0.85rem; display: inline-block; margin-right: 5px; }
        .tag-takeaway { background: #000; color: #fff; }
        .tag-dinein { background: #fff; color: #000; border: 2px solid #000; }

        .mods-box { margin-top: 2px; }
        .mod-bad { color: #d32f2f; font-weight: bold; }
        .mod-good { color: #2e7d32; font-weight: bold; }
        .note-box { background: #fff3cd; color: #856404; padding: 5px; margin-top: 5px; border-radius: 4px; font-size: 0.9rem; font-style: italic; border: 1px solid #ffeeba; }

        .late-warning { animation: pulse-red 1.5s infinite; border: 4px solid #ff0000; }
        ...
        .medium-warning { border: 4px solid #ffc107; }
    </style>
</head>
<body>
    <div class="row h-100 g-0">
        <div class="col-6 station-col">
            ...
            <?php foreach ($listaPizza as $t) renderTicket($t); ?>
        </div>
        <div class="col-6 station-col">
            <div class="station-header header-cocina"><i class="fa-solid fa-burger me-2"></i> Cocina</div>
            <?php foreach ($listaCocina as $t) renderTicket($t); ?>
        </div>
    </div>
</body>
</html>
...
function renderTicket($data) {
    ...
    // Umbrales de alerta
    if ($mins > 25) $borderClass = 'late-warning';
    elseif ($mins > 15) $borderClass = 'medium-warning';
                ...
                ?>
                    <div class="sub-item">
                        <div>
                            ...
                            <?php if($sub['name']): ?><span class="text-decoration-underline"><?= strtoupper($sub['name']) ?></span><?php endif; ?>
                        </div>
                        ...
                        <?php endif; ?>
                        <?php if(!empty($sub['note'])): ?><div class="note-box">⚠️ <?= $sub['note'] ?></div><?php endif; ?>
                    </div>
                <?php endforeach; ?>
            <?php endforeach; ?>
        </div>
