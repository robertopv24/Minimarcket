...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Procesar imagen
    $rutaImagen = 'default.jpg'; // Imagen por defecto
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $rutaDestino)) {
            $rutaImagen = 'uploads/product_images/' . $nombreImagen; // Ruta relativa para BD
        } else {
            $mensaje = '<div class="alert alert-warning">Error al subir la imagen. Se usará la predeterminada.</div>';
        }
    }
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="card bg-light mb-3">
                            <div class="card-body">
                                ...
                                <div class="row g-2">
                                    <div class="col-md-4">
                                        <label for="precio_usd" class="form-label fw-bold text-warning">Precio Venta ($)</label>
                                        <input type="number" class="form-control" id="precio_usd" name="precio_usd" step="0.01" required>
                                    </div>
                                    <div class="col-md-4">
                                        <label for="precio_ves" class="form-label text-warning">Precio Venta (Bs)</label>
                                        <input type="number" class="form-control" id="precio_ves" name="precio_ves" step="0.01" required>
                                    </div>
                                    <div class="col-md-4">
                                        <label for="profit_margin" class="form-label fw-bold text-warning">Margen (%)</label>
                                        <input type="number" class="form-control border-success" id="profit_margin" name="profit_margin" step="0.01" value="20.00" required>
                                        <small class="text-muted" style="font-size: 0.7rem;">Usado para recálculo automático</small>
                                    </div>
                                </div>
                            </div>
                        </div>
